...
import com.move_up.service.IUserService;
...
public class UserService extends BaseService implements IUserService {
	...
	@Override
	public UserDTO checkIn(HttpServletRequest request) {
		UserEntity requestedUserEntity = this.getRequestedUser(request);
		if (requestedUserEntity == null) 
			return (UserDTO)this.ExceptionObject(new UserDTO(), "Không tồn tại người dùng này.");
		
		// check if user checked-in already
		...
		for (MissionEntity missionEntity : missionEntities) 
			if (missionEntity.getType().equals("DIEMDANH"))
				return (UserDTO)this.ExceptionObject(new UserDTO(), "Bạn đã điểm danh rồi.");
		
		MissionEntity checkInMissionEntity = new MissionEntity();
		missionEntities = missionRepo.findAll();
		for (MissionEntity missionEntity : missionEntities) 
			if (missionEntity.getType().equals("DIEMDANH")) {
				checkInMissionEntity = missionEntity;
				requestedUserEntity.getMissions().add(missionEntity);
				break;
			}
		
		requestedUserEntity.setNumOfCoinGiftBox(requestedUserEntity.getNumOfCoinGiftBox() + checkInMissionEntity.getNumOfCoinGiftBox());
		requestedUserEntity.setNumOfTimeGiftBox(requestedUserEntity.getNumOfTimeGiftBox() + checkInMissionEntity.getNumOfTimeGiftBox());
		requestedUserEntity.setNumOfStar(requestedUserEntity.getNumOfStar() + checkInMissionEntity.getNumOfStar());
		...
		userDto.setMessage("Điểm danh thành công.");
		return userDto;
	}
